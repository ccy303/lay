...
import { logout, userInfo, config } from '@http/user';
...
const Header = ({ toggle, collapsed, CommonStore }) => {
  ...
  const logoutFn = async () => {
    try {
      ...
      // sessionStorage.removeItem('useAuth');
      // await userInfo();
      Message.success('登出成功');
      location.href = `${url}/login/logout?logout_redirect_uri=${encodeURI(location.origin)}`;
      // history.push('/login');
    } catch (error) {
      Message.error('登出失败');
    }
